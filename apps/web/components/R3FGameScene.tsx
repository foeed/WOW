...
function FlightObjects({ progress, playing }: { progress: number; playing: boolean }) {
...
export function R3FGameScene({ progress, playing }: R3FGameSceneProps) {
  return (
    <Canvas className="h-full w-full" dpr={[1, 2]} camera={{ position: [0, 0.3, 9.5], fov: 44 }}>
      ...
      <FlightObjects progress={progress} playing={playing} />
    </Canvas>
  );
}
